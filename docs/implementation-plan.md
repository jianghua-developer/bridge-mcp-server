...
- [ ] init git + `pyproject.toml`（uv，FastMCP，非发布型）+ **目录骨架（DESIGN §3.1）**：`bridge_mcp/` 共享胶水（config/bridge_cli/protocol/selection）+ `servers/generation/`（主线入口 + templates.yaml）+ `servers/governance/`（占位 README，不写代码）；server 配置声明桥 cli 命令路径（`cli.py` 或 `dist/bridge`）。
